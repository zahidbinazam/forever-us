...
// Parametric Lemniscate of Bernoulli equation for ∞ path
...
// Photo Orbits state
let orbitT1 = 0;
let orbitT2 = Math.PI; // Opposite side on ∞ loop

function animateInfinityHero() {
  infCtx.clearRect(0, 0, infWidth, infHeight);

  // Draw glowing background ∞ path line
  infCtx.save();
  infCtx.beginPath();
  const scaleX = infWidth * 0.38;
  const scaleY = infHeight * 0.45;
  ...
  // Update Orbiting Circular Photos DOM positioning
  orbitT1 += 0.005;
  orbitT2 += 0.005;
...
function initMainFeatures() {
  // Autoplay Music on first user unlock interaction
  ...
  // Start Live Relationship Countdown Counter
  initCountdown();
}
